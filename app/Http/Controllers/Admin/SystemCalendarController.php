...
class SystemCalendarController extends Controller
{
    ...
    {
        $bookings = [];
        $scoters = Scoter::all()->pluck('scoter_number', 'id');
        $customers = Customer::all()->pluck('full_name', 'id');

        foreach ($this->sources as $source) {
            $models = $source['model']::when($request->input('scoter_id'), function ($query) use ($request) {
                    $query->where('scoter_id', $request->input('scoter_id'));
                })
        ...
        return view('admin.calendar.calendar', compact('bookings', 'scoters', 'customers'));
